...
def get_parameters():
    f = open('sir_contamination_parameters.csv', 'r')
    mapping = dict()
    for line in f:
        data = line.split(',')
...
def sir(D, H, n, a, K, r, nb_mb, _e):
    ...
    def e(t):
        return _e + _e*math.sin(2*math.pi*t*dt/365)

    # Approximations
    for i in range(N_t):
        S[i+1] = S[i] + dt*(n*(H - S[i]) - a*S[i]*B[i]/(K + B[i]))
        I[i+1] = I[i] + dt*(a*B[i]*S[i]/(K + B[i]) - r*I[i])
        B[i+1] = B[i] + dt*(B[i]*(nb_mb) + e(i)*I[i])
...
def display_sir(S, I, B, t, D, H, n, a, K, r, nb_mb, e):
    ...
    ax.set_title('SIR Contamination Model')
