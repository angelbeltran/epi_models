...
def get_parameters():
    f = open('sir_extinction_parameters.csv', 'r')
    mapping = dict()
    for line in f:
        data = line.split(',')
...
def sir(D, H, n, a, K, r, _nb_mb, e):
    ...
    def nb_mb(t):
        return _nb_mb + _nb_mb*math.sin(2*math.pi*t*dt/365)

    # Approximations
    for i in range(N_t):
        S[i+1] = S[i] + dt*(n*(H - S[i]) - a*S[i]*B[i]/(K + B[i]))
        I[i+1] = I[i] + dt*(a*B[i]*S[i]/(K + B[i]) - r*I[i])
        B[i+1] = B[i] + dt*(B[i]*nb_mb(i) + e*I[i])
...
def display_sir(S, I, B, t, D, H, n, a, K, r, nb_mb, e):
    ...
    ax.set_title('SIR Extinction Model')
